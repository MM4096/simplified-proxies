	...
	return (<div className={`${className} card-list max-h-full overflow-y-auto`}>
		<h2 className="custom-divider">Card List</h2>
		{
			cards.map((card, index) => {
				return (<div className={`btn border border-black p-3 rounded-xl flex flex-row items-center text-left ${index === editingIndex && "bg-gray-300"}`} key={index}
				onClick={() => {
		...
		<button className={`border border-black rounded-xl btn ${(editingIndex === null || editingIndex < 0) && "bg-gray-300"}`} onClick={newCard}>New Card</button>
	</div>)
}